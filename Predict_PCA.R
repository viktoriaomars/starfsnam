...
#run a decision tree
#pca.train$GT <- as.factor(pca.train$GT) #sleppa şessari?

#library(rpart)
#rpart.model <- rpart(GT ~ .,data = train.data, method = "anova")
#rpart.model

library(tree)
tree.model <- tree(GT ~ .,data = train.data)
...
write.csv(test, "pc_prediction.csv", row.names = F)
